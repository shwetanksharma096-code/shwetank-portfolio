...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#111111', font = '30px sans-serif' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class AppCore {
  ...
  constructor(
    ...
    if (this.medias && this.medias.length > 0) {
      const initialScroll = (this.viewport.width / 2) - (this.medias[0].width / 2);
      this.scroll.current = this.scroll.target = this.scroll.last = initialScroll;
    }
    this.update();
  ...
  createMedias(
    ...
    const defaultItems = [
      ...
      { image: 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?q=80&w=800&auto=format&fit=crop', text: 'INDIAN IDOL' },
      { image: 'https://images.unsplash.com/photo-1519741497674-611481863552?q=80&w=800&auto=format&fit=crop', text: 'GOLD AWARDS' },
      { image: 'https://images.unsplash.com/photo-1501386761578-eac5c94b800a?q=80&w=800&auto=format&fit=crop', text: 'PVR CINEMAS' }
    ];
  ...
  update() {
    // Auto-scroll: continuously advance target when user is not dragging
    if (!this.isDown) {
